<template>
    <div class="main">
        <el-table :data="data.excerpts" height="85vh" style="width: 100%">
            <el-table-column type="index" width="50"/>
            <el-table-column prop="id" label="ID" width="50" />
            <el-table-column prop="content" label="内容" width="700" :show-overflow-tooltip='true'/>
            <el-table-column prop="provenance" label="出处" width="250" />
            <el-table-column prop="remark" label="备注" />
            <el-table-column fixed="right" label="Operations" width="120">
                <template #default="scope">
                    <el-button link type="primary" @click="modify(scope.row)">修改</el-button>
                    <el-popconfirm title="确定要删除吗?" @confirm="deleteexcerpt(scope.row.id)">
                        <template #reference>
        ...
        <el-dialog v-model="data.modifydialog" title="修改">
            ...
            <span>出处:</span>
            <el-input v-model="data.modifydata.provenance" />
            <span>备注:</span>
        ...
        <el-dialog v-model="data.adddialog" title="添加">
            ...
            <span>出处:</span>
            <el-input v-model="data.adddata.provenance" />
            <span>备注:</span>
    ...
    setup() {
        ...
        onMounted(() => {
            axios.post("api/excerpt/", {
                ...
                .then(function (value) {
                    data.excerpts = value.data.retlist;
                })
        ...
        function deleteexcerpt(id) {
            axios.post("api/excerpt/", {
                ...
                if (response.data.ret == 0) {
                    ...
                    data.excerpts = data.excerpts.filter((excerpt) => { return excerpt.id != id })   //从数组中去掉被删掉的数据
        ...
        function modifysubmit() {
            axios.post("api/excerpt/", {
                "action": "modify_excerpt",
                "id": data.modifydata.id,
                "newdata": {
                    "content": data.modifydata.content,
                    "provenance": data.modifydata.provenance,
        ...
        function addsubmit() {
            axios.post("api/excerpt/", {
                ...
                if (response.data.ret == 0) {
                    ...
                    const excerptObj = { id: response.data.id, content: data.adddata.content, provenance: data.adddata.provenance, remark: data.adddata.remark };
                    data.excerpts.push(excerptObj)
                    data.adddata.content = ""
                    data.adddata.provenance = ""
                    data.adddata.remark = ""
                } else if (response.data.ret == 302) {
                    alert("未登录");
                    router.push("signin");
